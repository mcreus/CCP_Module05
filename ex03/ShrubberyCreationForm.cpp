...
/*   Updated: 2023/10/19 10:26:44 by mcreus           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ShrubberyCreationForm::execute(Bureaucrat const &executor) const
{
	if (executor.getGrade() > 145)
		std::cout << executor.getName() << " have not sign and not execute ShruberryCreationForm because grade of bureaucrat is too low!" << std::endl;
	else if (executor.getGrade() < 145 && executor.getGrade() > 137)
		std::cout << executor.getName() << " have sign but not execute ShruberryCreationForm because grade of bureaucrat is too low!" << std::endl;
	else
	{

...
/*   Updated: 2023/10/19 10:25:52 by mcreus           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void    PresidentialPardonForm::execute(Bureaucrat const &executor) const
{
	if (executor.getGrade() > 25)
		std::cout << executor.getName() << " have not sign and not execute PresidentialPardonForm because grade of bureaucrat is too low!" << std::endl;
	else if (executor.getGrade() < 25 && executor.getGrade() > 5)
		std::cout << executor.getName() << " have sign but not execute PresidentialPardonForm because grade of bureaucrat is too low!" << std::endl;
	else
